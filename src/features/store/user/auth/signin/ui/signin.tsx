...
interface Props {
  login(email: string, password: string, products: ProductCart[]): void
}
export const SignIn: React.FC<Props> = ({ login }) => {
  ...
  function submitSignin(user: DataSignin, products: ProductCart[]) {
    try {
      login(user.email, user.password, products)
    } catch (error) {}
    history.push('/')
  }
          ...
          <form className={cx('form-signin')}>
            ...
            <div className={cx('botonera-form')}>
              <Button
                theme={'primary'}
                className="btn-100"
                onClick={() => submitSignin(user, products)}
              >
                Sign in
              </Button>
            </div>
            ...
            <div className={cx('botonera-form')}>
              <div className={cx('signin-google')}>
                <Button
                  ...
                  className="btn-100"
                  icon={
                    <Icon type="material-icons" content={'android'} title="Select price range" />
                  }
                >
                  Signup with Google
                </Button>
